...
export async function sendEmailSubscription(email) {
    ...
    try {
        ...
        return (response.data.status === "pending") ? {type: 'success',message:'Thank you for subscribing to In The Know YYC newsletters.'} : {type: 'error',message:'There was an error in the subscription. Please, try again later.'}
    } catch (error) {
